...
syn match 	cConstant	display "\<[A-Z0-9_]\+\>"
...
syn match	cType		display "\w\+\ze\s\+\([*]*\)\?[a-zA-Z_]"
syn match	cType		display "\(^\|}\)\@<=\s*\zs\w\+\ze\s*;"
syn match 	cType		display "(\@<=\s*\w*\ze\s*\([*]*\|\[.*\]\)\?)\s*\({\|\([&* ]*\)\?\s*\w\)"
syn match 	cIdentifier	display "\(\.\|->\)\zs\w\+"
syn match 	cDefine		display "#\(define\|pragma\|ifndef\|ifdef\|if\|elif\|else\|endif\)"
...
hi def link cTypedef		cStructure
